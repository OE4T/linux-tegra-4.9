...
int aq_nic_cfg_start(struct aq_nic_s *self)
{
	...
	if (cfg->is_interrupt_moderation) {
		/* use adaptive ITR by default */
		cfg->aq_itr.rx_adaptive = true;
		cfg->aq_itr.tx_adaptive = true;
	}
...
static void aq_nic_service_timer_cb(unsigned long param)
{
	...
	u64_stats_update_begin(&self->syncp);
	...
	u64_stats_update_end(&self->syncp);
				   ...
				   const struct aq_hw_ops *aq_hw_ops)
{
	...
	ndev->max_mtu = 16352; /* from MSM spec */
#endif
...
struct aq_nic_s *aq_nic_alloc_hot(struct net_device *ndev)
{
	...
	for (self->aq_vecs = 0; self->aq_vecs < self->aq_nic_cfg.vecs;
		...
		self->aq_vec[self->aq_vecs] =
		    aq_vec_alloc(self, self->aq_vecs, &self->aq_nic_cfg, self->aq_hw);
...
int aq_nic_xmit(struct aq_nic_s *self, struct sk_buff *skb)
{
	...
	return aq_ring_transmit_skb(ring, skb);
}
