...
{
	u32 dma_desc_addr_lsw = (u32)aq_ring->dma;
	u32 dma_desc_addr_msw = (u32)(((u64)aq_ring->dma) >> 32);
...
static int hw_atl_b0_hw_ring_tx_init(struct aq_hw_s *self,
				     ...
				     struct aq_ring_param_s *aq_ring_param)
{
	u32 dma_desc_lsw_addr = (u32)aq_ring->dma;
	u32 dma_desc_msw_addr = (u32)(((u64)aq_ring->dma) >> 32);
	...
	reg_tx_dma_desc_tail_ptr_set(self, 0, aq_ring->idx);
						 ...
						 bool itr_enabled)
{
	...
	unsigned int speed_index = hw_atl_utils_mbps_2_speed_index(
						self->aq_link_status.mbps);

	if (self->aq_nic_cfg->aq_itr.rx_adaptive) {
		...
		PHAL_ATLANTIC->itr_rx = 2U;
	
		PHAL_ATLANTIC->itr_rx |=
			hw_atl_b0_timers_table_rx_[speed_index]
			[0] << 0x8U; /* set min timer value */
		PHAL_ATLANTIC->itr_rx |=
			hw_atl_b0_timers_table_rx_[speed_index]
			[1] << 0x10U; /* set max timer value */
	} else if (cfg->aq_itr.rx_usecs) {
		unsigned int max_timer = cfg->aq_itr.rx_usecs / 2U;
		unsigned int min_timer = cfg->aq_itr.rx_usecs / 32U;

		max_timer = min(0x1FFU, max_timer);
		min_timer = min(0xFFU, min_timer);

		PHAL_ATLANTIC->itr_rx |= min_timer << 0x8U;
		PHAL_ATLANTIC->itr_rx |= max_timer << 0x10U;
	} else {
		PHAL_ATLANTIC->itr_rx = 0U;
	}

	if (self->aq_nic_cfg->aq_itr.tx_adaptive) {
		...
		PHAL_ATLANTIC->itr_tx = 2U;

		PHAL_ATLANTIC->itr_tx |=
			hw_atl_b0_timers_table_tx_[speed_index]
			[0] << 0x8U; /* set min timer value */
		PHAL_ATLANTIC->itr_tx |=
			hw_atl_b0_timers_table_tx_[speed_index]
			[1] << 0x10U; /* set max timer value */
	} else if (cfg->aq_itr.tx_usecs) {
		unsigned int max_timer = cfg->aq_itr.tx_usecs / 2U;
		unsigned int min_timer = cfg->aq_itr.tx_usecs / 32U;

		max_timer = min(0x1FFU, max_timer);
		min_timer = min(0xFFU, min_timer);

		PHAL_ATLANTIC->itr_tx |= min_timer << 0x8U;
		PHAL_ATLANTIC->itr_tx |= max_timer << 0x10U;
	} else {
		PHAL_ATLANTIC->itr_tx = 0U;
	}
	
	tdm_tx_desc_wr_wb_irq_en_set(self, PHAL_ATLANTIC->itr_tx ? 0U : 1U);
	tdm_tdm_intr_moder_en_set(self, PHAL_ATLANTIC->itr_tx ? 1U : 0U);
	rdm_rx_desc_wr_wb_irq_en_set(self, PHAL_ATLANTIC->itr_rx ? 0U : 1U);
	rdm_rdm_intr_moder_en_set(self, PHAL_ATLANTIC->itr_rx ? 1U : 0U);

	for (i = HW_ATL_B0_RINGS_MAX; i--;) {
		reg_tx_intr_moder_ctrl_set(self,  PHAL_ATLANTIC->itr_tx, i);
		reg_rx_intr_moder_ctrl_set(self,  PHAL_ATLANTIC->itr_rx, i);
	}
