				 ...
				 struct aq_hw_caps_s *aq_hw_caps)
{
	...
	AQ_HW_WAIT_FOR(0U != (PHAL_ATLANTIC->mbox_addr =
	...
	if (err < 0)
		printk(KERN_ALERT "aquantia: Bad firmware detected."
			" Expected version=%x, actual=%x",
				 ...
				 struct hw_aq_atl_utils_mbox *pmbox)
{
	...
	if (IS_CHIP_FEATURE(REVISION_A0)) {
		...
		pmbox->stats.dpc = atomic_read(&PHAL_ATLANTIC->dpc);
	} else {
