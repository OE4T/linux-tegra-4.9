...
enum tegra194_function {
	TEGRA194_FUNC_HSIC = 0,
	TEGRA194_FUNC_XUSB,
};
...
int tegra19x_phy_xusb_utmi_vbus_power_on(struct phy *phy);
int tegra19x_phy_xusb_utmi_vbus_power_off(struct phy *phy);
static int tegra194_padctl_regulators_init(struct tegra_padctl *padctl)
{
...
static int tegra194_padctl_parse_subnode(struct tegra_padctl *padctl,
				       ...
				       unsigned int *num_maps)
{
	...
	*maps = NULL;
	...
	for_each_child_of_node(parent, np) {
		...
		err = tegra194_padctl_parse_subnode(padctl, np, maps,
						    &reserved_maps,
						    num_maps);
		if (err < 0) {
...
static int tegra194_padctl_get_functions_count(struct pinctrl_dev *pinctrl)
{
	struct tegra_padctl *padctl = pinctrl_dev_get_drvdata(pinctrl);
...
static const char *
tegra194_padctl_get_function_name(struct pinctrl_dev *pinctrl,
...
static int tegra194_padctl_get_function_groups(struct pinctrl_dev *pinctrl,
					       unsigned int function,
					       const char * const **groups,
...
static int tegra194_padctl_pinmux_set(struct pinctrl_dev *pinctrl,
				      unsigned int function,
				      unsigned int group)
{
	struct tegra_padctl *padctl = pinctrl_dev_get_drvdata(pinctrl);
	...
	if (pad_is_otg(group)) {
		...
		if (function != TEGRA194_FUNC_XUSB)
...
static const struct pinmux_ops tegra194_padctl_pinmux_ops = {
	.get_functions_count = tegra194_padctl_get_functions_count,
	.get_function_name = tegra194_padctl_get_function_name,
	.get_function_groups = tegra194_padctl_get_function_groups,
	.set_mux = tegra194_padctl_pinmux_set,
};
...
static int tegra194_usb3_phy_enable_wakelogic(struct tegra_padctl *padctl,
					      int port)
{
	struct device *dev = padctl->dev;
	u32 reg;
...
static int tegra194_usb3_phy_disable_wakelogic(struct tegra_padctl *padctl,
					      int port)
{
	struct device *dev = padctl->dev;
	u32 reg;
...
static int tegra194_utmi_phy_enable_sleepwalk(struct tegra_padctl *padctl,
...
static int tegra194_utmi_phy_disable_sleepwalk(struct tegra_padctl *padctl,
					       int port)
{
	u32 reg;
...
static void tegra194_utmi_bias_pad_power_on(struct tegra_padctl *padctl)
{
	u32 reg;
...
static void tegra194_utmi_bias_pad_power_off(struct tegra_padctl *padctl)
{
	u32 reg;
...
void tegra19x_phy_xusb_utmi_pad_power_on(struct phy *phy)
{
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_power_on);

void tegra19x_phy_xusb_utmi_pad_power_down(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_power_down);
...
int tegra19x_utmi_vbus_enable(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	int port = utmi_phy_to_port(phy);
	int rc;
...
EXPORT_SYMBOL_GPL(tegra19x_utmi_vbus_enable);
...
static int tegra194_cdp_phy_set_cdp(struct phy *phy, bool enable)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
...
static int tegra194_cdp_phy_power_on(struct phy *phy)
{
	return tegra194_cdp_phy_set_cdp(phy, true);
}

static int tegra194_cdp_phy_power_off(struct phy *phy)
{
	return tegra194_cdp_phy_set_cdp(phy, false);
}

static const struct phy_ops cdp_phy_ops = {
	.power_on = tegra194_cdp_phy_power_on,
	.power_off = tegra194_cdp_phy_power_off,
	.owner = THIS_MODULE,
};
...
static int tegra194_hsic_phy_pupd_set(struct tegra_padctl *padctl, int pad,
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	if (on)
		rc = tegra194_hsic_phy_pupd_set(padctl, 0, PUPD_IDLE);
	else
		rc = tegra194_hsic_phy_pupd_set(padctl, 0, PUPD_RESET);
		...
		struct device_attribute *attr, const char *buf, size_t n)
{
	...
	if (on && !padctl->otg_vbus_alwayson) {
		err = tegra19x_phy_xusb_utmi_vbus_power_on(
		...
		err = tegra19x_phy_xusb_utmi_vbus_power_off(
				padctl->utmi_phys[port]);
		if (!err)
			padctl->otg_vbus_alwayson = false;
		else
...
static int tegra194_hsic_phy_pretend_connected(struct tegra_padctl *padctl,
					       int port)
{
	struct device *dev = padctl->dev;
	struct tegra_xusb_mbox_msg msg;
...
static int tegra194_hsic_phy_enable_sleepwalk(struct tegra_padctl *padctl,
					      int port)
{
	u32 reg;
...
static int tegra194_hsic_phy_disable_sleepwalk(struct tegra_padctl *padctl,
					       int port)
{
	u32 reg;
...
static int tegra194_hsic_phy_power_on(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	struct device *dev = padctl->dev;
...
static int tegra194_hsic_phy_power_off(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	int port = hsic_phy_to_port(phy);
	int rc;
...
static int tegra194_hsic_phy_init(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	int port = hsic_phy_to_port(phy);
...
static int tegra194_hsic_phy_exit(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	int port = hsic_phy_to_port(phy);
...
static const struct phy_ops hsic_phy_ops = {
	.init = tegra194_hsic_phy_init,
	.exit = tegra194_hsic_phy_exit,
	.power_on = tegra194_hsic_phy_power_on,
	.power_off = tegra194_hsic_phy_power_off,
	.owner = THIS_MODULE,
};
...
static void tegra_xusb_phy_mbox_work(struct work_struct *work)
{
	...
	case MBOX_CMD_STOP_HSIC_IDLE:
		...
		if (msg->cmd == MBOX_CMD_START_HSIC_IDLE)
			tegra194_hsic_phy_pupd_set(padctl, 0, PUPD_IDLE);
		else
			tegra194_hsic_phy_pupd_set(padctl, 0, PUPD_DISABLE);
		break;
	default:
		break;
	}
...
static struct phy *tegra194_padctl_xlate(struct device *dev,
...
static const char * const tegra194_hsic_groups[] = {
	"hsic-0",
};

static const char * const tegra194_xusb_groups[] = {
	"otg-0",
	"otg-1",
	"otg-2",
	"cdp-0",
...
#define TEGRA194_FUNCTION(_name)					\
	{								\
		.name = #_name,						\
		.num_groups = ARRAY_SIZE(tegra194_##_name##_groups),	\
		.groups = tegra194_##_name##_groups,			\
	}

static struct tegra_padctl_function tegra194_functions[] = {
	TEGRA194_FUNCTION(hsic),
	TEGRA194_FUNCTION(xusb),
};

static const unsigned int tegra194_otg_functions[] = {
	TEGRA194_FUNC_XUSB
};

static const unsigned int tegra194_hsic_functions[] = {
	TEGRA194_FUNC_HSIC,
};

#define TEGRA194_PAD(_name, _offset, _shift, _mask, _funcs)	\
	{								\
		.name = _name,						\
		.offset = _offset,					\
		.shift = _shift,					\
		.mask = _mask,						\
		.num_funcs = ARRAY_SIZE(tegra194_##_funcs##_functions),	\
		.funcs = tegra194_##_funcs##_functions,			\
	}

static const struct tegra_padctl_pad tegra194_pads[] = {
	TEGRA194_PAD("otg-0",  0x004,  0, 0x3, otg),
	TEGRA194_PAD("otg-1",  0x004,  2, 0x3, otg),
	TEGRA194_PAD("otg-2",  0x004,  4, 0x3, otg),
	TEGRA194_PAD("hsic-0", 0x004, 20, 0x1, hsic),
...
static const struct tegra_padctl_soc tegra194_soc = {
	.num_pins = ARRAY_SIZE(tegra194_pins),
	.pins = tegra194_pins,
	.num_functions = ARRAY_SIZE(tegra194_functions),
...
static const struct of_device_id tegra_padctl_of_match[] = {
	{.compatible = "nvidia,tegra194-xusb-padctl", .data = &tegra194_soc},
	{ }
};
...
static void tegra194_padctl_init(struct tegra_padctl *padctl)
{
	int i;
	u32 reg;
...
static void tegra194_padctl_save(struct tegra_padctl *padctl)
{
...
static void tegra194_padctl_restore(struct tegra_padctl *padctl)
{
...
static int tegra194_padctl_suspend(struct device *dev)
{
	struct tegra_padctl *padctl = dev_get_drvdata(dev);

	dev_dbg(dev, "%s\n", __func__);

	tegra194_padctl_save(padctl);

	return 0;
}

static int tegra194_padctl_resume(struct device *dev)
{
	struct tegra_padctl *padctl = dev_get_drvdata(dev);

	dev_dbg(dev, "%s\n", __func__);

	tegra194_padctl_init(padctl);
	tegra194_padctl_restore(padctl);

	return 0;
}

static const struct dev_pm_ops tegra194_padctl_pm_ops = {
	.suspend_noirq = tegra194_padctl_suspend,
	.resume_noirq = tegra194_padctl_resume,
};

static int tegra194_padctl_probe(struct platform_device *pdev)
{
	struct tegra_padctl *padctl;
	...
	if (tegra_platform_is_silicon()) {
		...
		err = tegra194_padctl_regulators_init(padctl);
		if (err < 0)
			return err;
	...
	padctl->desc.pmxops = &tegra194_padctl_pinmux_ops;
...
static int tegra194_padctl_remove(struct platform_device *pdev)
{
	struct tegra_padctl *padctl = platform_get_drvdata(pdev);
	int i;
	int err;
...
static struct platform_driver tegra194_padctl_driver = {
	.driver = {
		.name = "tegra194-padctl",
		.of_match_table = tegra_padctl_of_match,
		.pm = &tegra194_padctl_pm_ops,
	},
	.probe = tegra194_padctl_probe,
	.remove = tegra194_padctl_remove,
};
module_platform_driver(tegra194_padctl_driver);

/* Tegra Generic PHY Extensions */
int tegra19x_phy_xusb_enable_sleepwalk(struct phy *phy,
				    enum usb_device_speed speed)
{
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_enable_sleepwalk(padctl, port, speed);
	} else if (is_hsic_phy(phy)) {
		port = hsic_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_enable_sleepwalk(padctl, port);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_enable_wakelogic(padctl, port);
	} else
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_enable_sleepwalk);

int tegra19x_phy_xusb_disable_sleepwalk(struct phy *phy)
{
	struct tegra_padctl *padctl = phy_get_drvdata(phy);
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_disable_sleepwalk(padctl, port);
	} else if (is_hsic_phy(phy)) {
		port = hsic_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_disable_sleepwalk(padctl, port);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_disable_wakelogic(padctl, port);
	} else
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_disable_sleepwalk);

static int tegra194_padctl_vbus_override(struct tegra_padctl *padctl,
					 bool on)
{
	u32 reg;
...
int tegra19x_phy_xusb_set_vbus_override(struct phy *phy)
{
	struct tegra_padctl *padctl;
	...
	return tegra194_padctl_vbus_override(padctl, true);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_set_vbus_override);

int tegra19x_phy_xusb_clear_vbus_override(struct phy *phy)
{
	struct tegra_padctl *padctl;
	...
	return tegra194_padctl_vbus_override(padctl, false);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_clear_vbus_override);

static int tegra194_padctl_id_override(struct tegra_padctl *padctl,
					 bool grounded)
{
	u32 reg;
...
int tegra19x_phy_xusb_set_id_override(struct phy *phy)
{
	struct tegra_padctl *padctl;
	...
	return tegra194_padctl_id_override(padctl, true);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_set_id_override);

int tegra19x_phy_xusb_clear_id_override(struct phy *phy)
{
	struct tegra_padctl *padctl;
	...
	return tegra194_padctl_id_override(padctl, false);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_clear_id_override);
...
bool tegra19x_phy_xusb_has_otg_cap(struct phy *phy)
{
	struct tegra_padctl *padctl;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_has_otg_cap);

static int tegra194_usb3_phy_set_wake(struct tegra_padctl *padctl,
...
static int tegra194_utmi_phy_set_wake(struct tegra_padctl *padctl,
				      int port, bool enable)
{
	u32 reg;
...
static int tegra194_hsic_phy_set_wake(struct tegra_padctl *padctl,
				      int port, bool enable)
{
	u32 reg;
...
int tegra19x_phy_xusb_enable_wake(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_set_wake(padctl, port, true);
	} else if (is_hsic_phy(phy)) {
		port = hsic_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_set_wake(padctl, port, true);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_set_wake(padctl, port, true);
	} else
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_enable_wake);

int tegra19x_phy_xusb_disable_wake(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_set_wake(padctl, port, false);
	} else if (is_hsic_phy(phy)) {
		port = hsic_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_set_wake(padctl, port, false);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_set_wake(padctl, port, false);
	}
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_disable_wake);


static int tegra194_usb3_phy_remote_wake_detected(struct tegra_padctl *padctl,
						  int port)
{
	u32 reg;
...
static int tegra194_utmi_phy_remote_wake_detected(struct tegra_padctl *padctl,
						  int port)
{
	u32 reg;
...
static int tegra194_hsic_phy_remote_wake_detected(struct tegra_padctl *padctl,
						  int port)
{
	u32 reg;
...
int tegra19x_phy_xusb_remote_wake_detected(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_remote_wake_detected(padctl, port);
	} else if (is_hsic_phy(phy)) {
		port = hsic_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_remote_wake_detected(padctl, port);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_remote_wake_detected(padctl, port);
	}
		return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_remote_wake_detected);


int tegra19x_phy_xusb_pretend_connected(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_hsic_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_hsic_phy_pretend_connected(padctl, port);
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_pretend_connected);

void tegra19x_phy_xusb_set_dcd_debounce_time(struct phy *phy, u32 val)
{
	struct tegra_padctl *padctl;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_set_dcd_debounce_time);

void tegra19x_phy_xusb_utmi_pad_charger_detect_on(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
	...
	tegra19x_phy_xusb_utmi_pad_power_on(phy);
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_charger_detect_on);

void tegra19x_phy_xusb_utmi_pad_charger_detect_off(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
	...
	tegra19x_phy_xusb_utmi_pad_power_down(phy);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_charger_detect_off);

void tegra19x_phy_xusb_utmi_pad_enable_detect_filters(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_enable_detect_filters);

void tegra19x_phy_xusb_utmi_pad_disable_detect_filters(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_disable_detect_filters);

void tegra19x_phy_xusb_utmi_pad_set_protection_level(struct phy *phy, int level,
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_set_protection_level);

bool tegra19x_phy_xusb_utmi_pad_dcd(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_dcd);

u32 tegra19x_phy_xusb_noncompliant_div_detect(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_noncompliant_div_detect);

bool tegra19x_phy_xusb_utmi_pad_primary_charger_detect(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_primary_charger_detect);

bool tegra19x_phy_xusb_utmi_pad_secondary_charger_detect(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_pad_secondary_charger_detect);
...
int tegra19x_phy_xusb_utmi_vbus_power_on(struct phy *phy)
{
	...
	if (padctl->oc_pinctrl && padctl->utmi_ports[port].oc_pin >= 0) {
		...
		tegra194_enable_vbus_oc(padctl->utmi_phys[port]);
	} else {
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_vbus_power_on);
...
int tegra19x_phy_xusb_utmi_vbus_power_off(struct phy *phy)
{
	...
	if (padctl->oc_pinctrl && padctl->utmi_ports[port].oc_pin >= 0) {
		...
		tegra194_disable_vbus_oc(padctl->utmi_phys[port]);
	} else {
		status = regulator_is_enabled(padctl->vbus[port]);
		if (padctl->vbus[port] && status) {
			rc = regulator_disable(padctl->vbus[port]);
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_utmi_vbus_power_off);

int tegra19x_phy_xusb_overcurrent_detected(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	bool detected = false;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_overcurrent_detected);

void tegra19x_phy_xusb_handle_overcurrent(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
	unsigned i;
	...
	for (i = 0; i < TEGRA_UTMI_PHYS; i++) {
		...
		if (reg & OC_DETECTED_VBUS_PAD(pin)) {
			dev_info(padctl->dev, "%s: clear port %d pin %d OC\n",
				 __func__, i, pin);
			tegra194_enable_vbus_oc(padctl->utmi_phys[i]);
		}
	}
	mutex_unlock(&padctl->lock);
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_handle_overcurrent);

static int tegra194_usb3_phy_reverse_id(struct tegra_padctl *padctl,
					int port, bool enable)
{
	u32 reg;
...
static int tegra194_utmi_phy_reverse_id(struct tegra_padctl *padctl,
					int port, bool enable)
{
	u32 reg;
...
int tegra19x_phy_xusb_set_reverse_id(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_reverse_id(padctl, port, true);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_reverse_id(padctl, port, true);
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_set_reverse_id);

int tegra19x_phy_xusb_clear_reverse_id(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_reverse_id(padctl, port, false);
	} else if (is_usb3_phy(phy)) {
		port = usb3_phy_to_port(phy);
		if (port < 0)
			return -EINVAL;
		return tegra194_usb3_phy_reverse_id(padctl, port, false);
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_clear_reverse_id);

int tegra19x_phy_xusb_generate_srp(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
	int port;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_generate_srp);

static int tegra194_utmi_phy_srp_detect(struct tegra_padctl *padctl,
					int port, bool enable)
{
	u32 reg;
...
int tegra19x_phy_xusb_enable_srp_detect(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_srp_detect(padctl, port, true);
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_enable_srp_detect);

int tegra19x_phy_xusb_disable_srp_detect(struct phy *phy)
{
	struct tegra_padctl *padctl;
	int port;
	...
	if (is_utmi_phy(phy)) {
		...
		if (port < 0)
			return -EINVAL;
		return tegra194_utmi_phy_srp_detect(padctl, port, false);
	}

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_disable_srp_detect);

bool tegra19x_phy_xusb_srp_detected(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
	int port;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_srp_detected);

int tegra19x_phy_xusb_enable_otg_int(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_enable_otg_int);

int tegra19x_phy_xusb_disable_otg_int(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_disable_otg_int);

int tegra19x_phy_xusb_ack_otg_int(struct phy *phy)
{
	struct tegra_padctl *padctl;
	u32 reg;
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_ack_otg_int);

int tegra19x_phy_xusb_get_otg_vbus_id(struct phy *phy,
...
EXPORT_SYMBOL_GPL(tegra19x_phy_xusb_get_otg_vbus_id);

MODULE_AUTHOR("JC Kuo <jckuo@nvidia.com>");
MODULE_DESCRIPTION("Tegra 194 XUSB PADCTL driver");
