...
static int stepper_fopen(struct inode *node, struct file *file)
{
	...
	struct stepper_device *stepper = container_of(cdev, struct stepper_device, chardev);
	stepper->offset = 0;
	return 0;
}
...
static loff_t stepper_seek(struct file *file, loff_t offset, int len)
{
	struct cdev *cdev = file->f_path.dentry->d_inode->i_cdev;
	struct stepper_device *stepper = container_of(cdev, struct stepper_device, chardev);
	stepper->offset = (int)offset;
	return offset;
}

static long stepper_fioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int status = 0;
	struct cdev *cdev = file->f_path.dentry->d_inode->i_cdev;
	struct stepper_device *stepper = container_of(cdev, struct stepper_device, chardev);
...
static ssize_t stepper_fwrite(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	int ret = count;
	struct cdev *cdev = file->f_path.dentry->d_inode->i_cdev;
	struct stepper_device *stepper = container_of(cdev, struct stepper_device, chardev);
...
static ssize_t stepper_fread(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int ret = count;
	struct cdev *cdev = file->f_path.dentry->d_inode->i_cdev;
	struct stepper_device *stepper = container_of(cdev, struct stepper_device, chardev);
...
{
	struct stepper_device *stepper = container_of(dev, struct stepper_device, dev);
	ida_simple_remove(&stepper_ida, stepper->id);
}

struct stepper_device *stepper_device_register(const char *name, struct device *dev,
					const struct stepper_ops *ops,
					struct module *owner)
{
	struct stepper_device *stepper;
	int err, of_id = -1, id = -1;
...
err:
	...
	dev_err(dev, "steper core: Error registering device: %s, err = %d", name, err);
	return ERR_PTR(err);
}
...
int stepper_motor_direction(struct stepper_device *stepper, enum stepper_direction dir)
{
	int err = 0;
	if (!stepper->ops) {
		err = -ENODEV;
	} else if (!stepper->ops->set_direction) {
		err = -EINVAL;
	} else {
		err = mutex_lock_interruptible(&stepper->ops_lock);
		if (!err) {
...
int stepper_motor_setrate(struct stepper_device *stepper, int val, enum stepper_rate_ramp immediate)
{
	int err = 0;
	if (!stepper->ops) {
		err = -ENODEV;
	} else if (!stepper->ops->set_rate) {
		err = -EINVAL;
	} else {
		err = mutex_lock_interruptible(&stepper->ops_lock);
		if (!err) {
...
int stepper_motor_set_ramp(struct stepper_device *stepper, int val, enum stepper_ramp_mode dir)
{
	int err = 0;
	if (!stepper->ops) {
		err = -ENODEV;
	} else if (!stepper->ops->set_ramp_rate) {
		err = -EINVAL;
	} else {
		err = mutex_lock_interruptible(&stepper->ops_lock);
		if (!err) {
...
static int __init stepper_init(void)
{
	...
	status = alloc_chrdev_region(&stepper_devt, 0, STEPPER_DEV_MAX, "stepper");
	return 0;
err:
...
static void __exit stepper_exit(void)
{
	...
	if (stepper_devt)
		unregister_chrdev_region(stepper_devt, STEPPER_DEV_MAX);
