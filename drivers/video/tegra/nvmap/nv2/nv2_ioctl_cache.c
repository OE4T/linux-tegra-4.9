					...
					u32 *handle_ptrs, int nr)
{
	...
	for (i = 0; i < nr; i++) {
		...
		handles[i] = NVMAP2_handle_get(handles[i]);
		if (!handles[i]) {
				 ...
				 bool is_reserve_ioctl)
{
	...
	if (is_reserve_ioctl) {
		err = NVMAP2_handles_reserve(handles, offset_ptr, size_ptr,
							op.op, op.nr);
	} else {
		err = NVMAP2_handles_cache_maint(handles, offset_ptr, size_ptr,
							op.op, op.nr);
	}
