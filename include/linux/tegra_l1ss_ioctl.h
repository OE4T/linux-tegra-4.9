...
/*
 * Defines the error status when fault is confirmed at source,
 * and is not fixed.
 */
#define NVGUARD_ERROR_DETECTED				(0xAAU)
/*
 * Defines the error status when the test function does not
 * encounter a fault,
...
/*
 * Maximum length of error information, in bytes, in service status structure
 */
...
#define NVGUARD_PHASE_NOTIFICATOIN		(0x08U)
